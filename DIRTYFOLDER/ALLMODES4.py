        ...
        if run_code("SAFEMODE.py"):
            choice = input("SAFE MODE HAS BEEN COMPLETED! WOULD YOU LIKE TO CONTINUE TO STANDBY MODE? (yes/no):").lower()
                ...
                if choice == "yes":
                    ...
                    run_code(code_file)
                    #if code_file == "REHYDRATIONMODE.py":
                    #    choice_rehydration  = input("REHYDRATION MODE is next.Would you like to continue? (yes/no):").lower()
                     #       if choice_rehydration = "yes":
            ...
            while True:
                code_choice = input("Which mode would you like to continue to ? (STANDBY/REHYDRATION/SCIENCE/DECOMMISSION):").upper()
...
def continue_execution(signal, frame):
user_choice = input ("You have aborted. Would you like to continue 
