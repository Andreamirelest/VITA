...
code_files = ["STANDBYMODE.py","SCIENCEMODE.py","REHYDRATEMODE.py","DECOMMISSIONMODE.py"]

#Function that runs code file and handles errors
...
def main():
    while True:
        ...
        while True:        
            choice = input("SAFE MODE HAS BEEN SUCCESFULLY COMPLETED! INDICATE WHETHER YOU WANT TO CONTINUE TO STANDBY MODE? (yes/no):").lower() 
        if choice == "yes"
           ...
           #choice = input("STANDBY MODE HAS BEEM SUCCESFULLY COMPLETED! INDICATE WHETHER YOU WANT TO CONTINUE TO REHYDRATION MODE? (yes/no)").lower
              #if choice == "yes":
               #   for code_file in code_files:
               #   run_code(code_files)
                #  break
              ...
              while True:
                  code_choice = input("Which mode would you like to continue to ? (STANDBY/SCIENCE/REHYDRATE/DECOMMISSION):").upper()
                  if code_choice in ["STANDBY","SCIENCE","REHYDRATE","DECOMMISSION" ]:
                      run_code(f"{code_choice.upper()}MODE.py")      #this allows to link the choices to the code_files document 
                  else:
                      print("INVALID CODE CHOICE.") 
   #for code_file in code_file[index:]:
                      #   if not run_code(code_file):
                       #      break 
                  break

            #print("ALL MODES HAVE SUCCESFULLY TERMINATED")
