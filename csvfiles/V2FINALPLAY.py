...
#TARGET TEMPERATURES
SetPoint = [30.0, 28.5, 32.0, 30.0]

#Gains for PID loops
Kp = [1,1,1]  #Proportional gains
Ki = [1,1,1]  #Integral gains 
Kd = [1,1,1]  #Derivative gains

#Temporary variables 

deriv = 0                           #Change since last measurement (derivative) error = 0                           #Difference between the target temperature (proportional)
prev  = [0, 0, 0, 0]                #Previous value for each PID loop
integral = [0, 0, 0, 0]             #integral for each PID loop                 
intMin = 0                          #minimum integral value (set to zero to avoid windup)
intMax = 20                         #maximum integral value
Temp = [5, 5, 5, 5]                 #current temperature for each PID loop
Duty = np.array([0, 0, 0, 0], dtype=np.uint8)     #Dutycycle values (0-255)
Time = 0                            #elapse time in seconds 
LoopUpdate = [0, 0, 0, 0]           #set to 1 when a target temperature has been updated 
                
               
HeaterAdresses= [0x01, 0x02, 0x04]
TempSensorAdresses= [0x18, 0x19, 0x1B]
...
#read temperatures from sensors 
           for i in range(3) :
               ...
               tempRaw16 = bus.read_i2c_block_data(TempSensorAdresses[i], 0x05, 2)
               tempRaw = (tempRaw16[0] << 8) | tempRaw16[1]
               if tempRaw != 0xFFFF:
                   tempC = tempRaw & 0x0FFF
                   tempC /= 16.0
                   if tempRaw & 0x1000:
                       tempC -= 256
               Temp[i] = tempC
