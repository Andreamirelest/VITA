...
def run_code(filename):
    try:
        ...
        elif  isinstance(e, KeyboarInterrupt):
           print("Error type: Keyboard Interrupt") 
...
def main():
    while True:
        ...
        if choice == "yes":
          # for code_file in code_files:
            if not run_code("STANDBYMODE.py"):
                print ("Error occurred in STANDBYMODE. SAFE MODE WILL START AS DEFAULT MODE.") 
        ...
        if choice == "yes":
            if not run_code("REHYDRATIONMODE.py"):
                print ("Error occured in REHYDRATIONMODE. SAFEMODE WILL START AS DEFAULT MODE.")
                run_code ("SAFEMODE.py")
       ...
       # elif choice == "sos"
        ...
        if choice == "yes":
           if not run_code("SCIENCEMODE.py"):
               print ("Error in SCIENCE MODE. SAFEMODE WILL START AS DEFAULT MODE")
               run_code ("SAFEMODE.py")


           choice = input ("SCIENCE MODE HAS BEEN COMPLETED SUCCESFULLY! WOULD YOU LIKE TO CONTINUE TO DECOMMISSION MODE? (yes/no):").lower()
        if choice == "yes":
           if not run_code("DECOMMISSIONMODE.py"):
               print ("Error in DECOMMISSION MODE. SAFEMODE WILL START AS DEFAULT MODE")
               run_code ("SAFEMODE.py")

           print ("ALL MODES HAVE BEEN SUCCESFUL!")
           confirmation = input ("WOULD YOU LIKE TO EXIT? (yes/no):").lower()
           if confirmation == "yes":
               break
           ...
           break      
            ...
            while True:
               ...
               #     run_code(f"{code_choice.upper()}.py")
                ...
                if code_choice in ["SAFE","STANDBY","REHYDRATION","SCIENCE","DECOMMISSION" ]:
                    ...
                    if code_choice == "DECOMMISSION":
                        print ("ALL MODES HAVE BEEN COMPLETED!")
                        break
            ...
            #    run_code ("DECOMMISSIONMODE.py")    
             ...
             #      break             
                    ...
                    print("Invalid choice choose a valid mode") 
                      


        else:
            print ("ERROR HAS OCCURED SAFEMODE WILL START AS DEFAULT MODE")
            run_code ("SAFEMODE.py") 
